...
- **2026-09-08 실제 선진 규정 PDF 130개(pdftotext로 추출)를 표본 조사해 조항 경계 로직의 실전 공백
  두 가지를 보완했다.** (1) 전체의 약 20%(27개)가 "제1조"가 아니라 "제 1 조"처럼 숫자와 "조" 사이에
  공백을 넣는 표기를 쓰고 있었는데, 예전 정규식(`제\s*\d+조`)은 숫자 뒤 공백을 허용하지 않아 이 문서들은
  조항 경계를 하나도 못 찾고 전부 "문서 전체를 조항 하나로" 취급하는 폴백으로 빠지고 있었다
  (전산장비관리규정, 여비규정, 정보보호정책 등). `ARTICLE_PATTERN`을 `제\s*\d+\s*조(?:\s*의\s*\d+)?`로
  넓혀 이 표기도 인식하게 했다. (2) "부칙"(附則, 시행일·경과조치)은 표본의 절반 가까운 문서에서 "제N조"
  형식을 쓰지 않고 "1. (시행일)", "①(시행일)"처럼 별도 번호체계를 썼다 - 이 경우 부칙 전체가 마지막
  조항 청크 꼬리에 그대로 흡수되어(예: "제24조" 청크 안에 시행일 정보가 섞임) 시행일을 묻는 질의의 근거가
  흐려진다. `ADDENDUM_PATTERN`("부칙"을 별도 헤더로 인식, "부칙(문서명. 제정일)"처럼 괄호가 바로 붙는
  경우까지 포함)을 추가해 부칙을 조항과 별개의 청크 경계로 인정했다. 두 정규식 모두 매치 직후 한글이
  공백 없이 바로 붙으면("제17조의 시정요구에 대하여는...", "부칙은 본칙에 부수하는...") 제목이 아닌
  본문 인용으로 보고 제외하는 부정 전방탐색(`(?![가-힣])`)을 추가했다 - 표본 조사 중 실제로 "제18조
  (시정확인)\n제17조의 시정요구에 대하여는..."처럼 줄바꿈이 우연히 조항 인용 앞에서 끊긴 사례를
  발견했기 때문이다(이 케이스는 숫자-조 사이 공백이 없어 구버전 정규식에서도 이미 잘못 매치되던
  기존 버그였고, 이번에 같이 고쳐졌다). 다만 이렇게 패턴을 넓히면 목차(TOC) 페이지가 조항 패턴을
  흉내내는 경우가 새로 걸릴 위험이 있어("제10조 (표제) ....................... 3"처럼 점선 리더 뒤에
  페이지 번호를 붙이는 목차 - 정보보호정책류 12개 문서에서 확인), 매치된 줄의 나머지가 점 3개 이상 +
  숫자로 끝나면 목차 항목으로 보고 제외하는 `TOC_DOT_LEADER_SUFFIX` 필터를 추가했다 - 없었다면 목차
  한 페이지가 조항 수만큼 의미 없는 초소형 청크로 쪼개질 뻔했다. "부칙"이 목차에 번호 없이 나열되는
  경우("제1장 총칙 ... 부칙")도 있어, 첫 조항 시작 이전에 등장하는 "부칙" 매치는 목차로 보고 제외한다.
  이 조사로 "제N조" 패턴을 하나도 못 찾던 문서 비율이 130개 중 67개(52%)에서 49개(38%)로 줄었다.
- **2026-09-08 (이어서) 사용자가 "남은 49개도 분류가 안 되냐"고 재차 확인을 요청해, 49개를 다시 하나씩
  들여다보고 원인별로 재분류했다.** 그 결과 실제 버그 1건과, 표본에 1건뿐이라 전용 처리 여부를 사용자와
  상의해서 결정한 1건을 추가로 찾았다:
  - **버그: 조항 번호 앞에 들여쓰기 공백이 있으면 놓치고 있었다.** `사택 및 주거지원규정`이 " 제 1 조[목
    적]"처럼 줄 맨 앞에 공백을 넣는데, `ARTICLE_PATTERN`이 `^제`로 시작해 이 공백을 허용하지 않았다
    (`ADDENDUM_PATTERN`에는 이미 `^\s*`를 넣어놓고 조항 패턴에는 빼먹은 비대칭이었다). `^\s*제...`로
    통일해서 고쳤고, 이 문서의 조항 9개가 정상적으로 나뉘는 것을 확인했다. 부작용으로 산업안전보건법
    위반 과태료표 등 외부 법령 참조 "표" 문서 2건에서 표 셀 내용이 우연히 조항 패턴처럼 보여 스푸리어스
    매치가 하나씩 늘었는데, 이 문서들은 회사 자체 규정이 아니라 표 형태의 외부 참고자료라 애초에 조항
    단위 청킹의 이득이 없는 문서였으므로 감수하기로 했다.
  - **로마숫자+소수점 outline 매뉴얼용 2차 폴백을 추가했다.** `채권관리매뉴얼`(개정본 2벌)은 "제N조" 대신
    "Ⅰ. 담보평가 → 1. 적용범위 → 4.1 평가대상물건 사전조사 → 4.3.1.1 평가액 산정"처럼 로마숫자+소수점
    5단 outline을 쓴다. 소수점 항목("1.", "4.1" 등)은 다른 모든 문서에서도 흔한 일반 목록 표기라 전역
    경계로 쓰면 오탐이 너무 많아서, 로마숫자 대제목(Ⅰ.~Ⅹ.)만 경계로 인정하고, 그마저도 `ARTICLE_PATTERN`/
    `ADDENDUM_PATTERN`이 경계를 하나도 못 찾은 문서에 한해서만(순수 2차 폴백) 시도하도록 범위를 좁혔다
    (`findRomanNumeralBoundaries`). 이 매뉴얼의 목차가 2단 레이아웃이라 "Ⅰ. 담보평가        2. 거래의
    개설"처럼 같은 줄에 다른 항목이 이어붙는데, 매치된 줄에 공백 4칸 이상 뒤에 다른 글자가 있으면 목차의
    병합된 줄로 보고 제외하는 필터(`MULTI_COLUMN_TOC_GAP`)도 추가했다 - 완벽하진 않아서(오른쪽 칸이
    우연히 빈 목차 행 1개는 걸러지지 않고 통과함) 실제 검증 결과 로마숫자 대제목 10개 중 9개는 정확히
    인식되고 목차 잔재 1개만 작은 스푸리어스 청크로 남는데, 이 정도는 "문서 전체가 챕터 구분 없이 하나로
    토큰 분할되던" 기존 상태보다 명백히 개선이라 사용자와 상의 후 반영했다. 표본 130개 중 이 문서(2벌)
    에만 해당하는 좁은 케이스라, 이후 같은 로마숫자 outline 형식의 문서가 더 늘어나면 그때 다시 다듬을 것.
  - **나머지는 실제로 조/항 구조가 없는 문서였다**: (a) 스캔본(본문이 이미지라 텍스트 추출 자체가 거의
    안 됨) - 공시정보관리규정, 징계규정, 전산장비관리운영규정(사본) 3건, OCR 없이는 손댈 수 없음.
    (b) PDF 자체가 손상되어 파서가 열지도 못하는 파일 3건 - 기존 코드가 이미 400으로 거부 중이라 원본을
    다시 내보내야 하는 문제. (c) 표/조직도/이미지 위주 문서(비상연락망, 과태료표, 유형표, CI/캐릭터
    디자인 가이드·배너 등) 약 25건 - 조항 경계 자체가 없어 정규식으로 도울 수 없음(표 추출은 별도 과제).
    (d) 윤리헌장/강령/실천지침/슬로건 등 짧은 선언문 5건 - 1~2페이지라 굳이 안 쪼개도 무방. (e) 고용노동부
    발간 안내서/매뉴얼 3건 - 회사 자체 규정이 아닌 외부 참고자료. 이 카테고리들은 "기존 폴백(문서 전체를
    하나로 취급 후 토큰 분할)이 그대로 적용되는 게 맞는 케이스"라 판단해 추가로 손대지 않았다.
- **2026-09-08 (이어서) 위 (c) 25건 중 "표/조직도"류와 "순수 이미지 배너"류는 서로 다른 문제라는 걸
  사용자와 함께 확인하고, 그중 순수 이미지 배너만 지금 처리했다.** 실측 결과 과태료·벌칙표(`7-13.`)는
  pdftotext로 92,945바이트가 뽑히는 등 **텍스트 자체는 이미 다 들어있고** 표의 시각적 행/열 배치가
  줄 단위로 뒤섞이는 게 문제라 OCR이 전혀 도움이 안 되는 반면, POPO 캐릭터 공고문 배너 PDF는 8바이트밖에
  안 뽑혀서 사실상 순수 이미지다(회사 규정 Q&A에 색인할 실익도 없음). 표 재인식(좌표 기반 행/열 재구성)은
  범위가 커서 별도 논의 후 진행하기로 하고, 순수 이미지 배너 감지·제외만 반영했다. `DocumentIngestionService
  .ingestOne()`에서 `readDocuments()` 직후 추출된 텍스트 총량을 검사해 50자 미만이면
  `IllegalArgumentException`을 던진다 - 가장 짧은 실제 규정(윤리헌장류 선언문)도 1,500자 이상은 나와서
  이 정도로도 충분히 구분된다. 이 예외는 단일 업로드에서는 기존 `GlobalExceptionHandler`가 400으로
  응답하고, zip 일괄 업로드에서는 `ingestZip()`이 이미 `IllegalArgumentException`을 "건너뛴 항목"으로
  잡아 사유와 함께 보고하는 기존 경로를 그대로 탄다(새 예외 처리 경로를 따로 만들지 않았다). 스캔본(본문이
  이미지인 실제 규정 PDF, 위 (a))은 이 검사로도 걸러지긴 하지만 원인이 다른 별개 문제(OCR 필요)이므로
  혼동하지 않도록 구분해서 기록해둔다.
